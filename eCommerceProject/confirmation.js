let confirmedItems = document.getElementById('confirmedItems');
let billableCost = document.getElementById("billableCost")
//let basket1 = JSON.parse(localStorage.getItem("data1")) || [];

calculation();
let billableAmount = ()=>{
    let total = JSON.parse(localStorage.getItem("Total")) || [];
            let shipCharge = JSON.parse(localStorage.getItem("ShipCost")) || [];
    billableCost.innerHTML = `
    <p>Shipping Charge: $ ${shipCharge}</p>
    <p>Total Bill Amount: $ ${total}</p>`
}
billableAmount();
let generateConfirmedItems = () =>{
    if(basket.length !== 0){
        //confirmedItems.innerHTML = "Not empty";
        return (confirmedItems.innerHTML = basket.map((x)=>{
            let { id, item } = x;
            
            let search = shopItemsData.find((y) => y.id === id) || [];
            return `<h4 class="confirmDetails">
            <p>Item Name: ${search.name}</p><br>
            <p class="Number-of-Items"> Number of Items: ${item}</p><br>
            <p class="tax">Tax Percent: ${search.tax}</p><br>
            <p>$ Price Per Piece: ${(search.price)}</p>
            <p>$ Total Price: ${(item * search.price) + ((item * search.price) * search.tax /100)}</p>
        </h4> `
...
// let shippingCost = document.getElementById("shipCost").value;
// let shippingCost1 = document.getElementById("shipCost1").value;
